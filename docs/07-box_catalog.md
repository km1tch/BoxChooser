...
5. **Store-Accessible Vendor Endpoints**
   ...
   - `/api/store/{store_id}/boxes` - Create boxes from vendor catalog
...
3. **Full Onboarding Wizard**
   - Have the pieces but not composed into a wizard
   - Current "Add Boxes" flow is sufficient for now
   - Could be assembled into a guided wizard later if needed
...
### 1. CLI Tool for Vendor Updates (Maybe...)

Create tools to handle vendor catalog updates:
...
Note: For now, manual YAML editing and git diff may be sufficient given the low frequency of vendor catalog updates.
