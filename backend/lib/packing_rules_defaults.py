...
try:
    ...
    logger.critical(f"Could not load required packing_guidelines.yml: {e}")
    sys.exit(1)

# Validate required sections exist
if 'recommendation_engine' not in _guidelines_data:
    logger.critical("packing_guidelines.yml missing 'recommendation_engine' section")
    sys.exit(1)
...
for field in required_engine_fields:
    if field not in DEFAULT_ENGINE_CONFIG:
        logger.critical(f"packing_guidelines.yml missing 'recommendation_engine.{field}'")
        sys.exit(1)
...
for packing_type in ['basic', 'standard', 'fragile', 'custom']:
    if packing_type not in _guidelines_data:
        logger.critical(f"packing_guidelines.yml missing '{packing_type}' section")
        sys.exit(1)
    ...
    for field in required_rule_fields:
        if field not in rule:
            logger.critical(f"packing_guidelines.yml missing '{packing_type}.{field}'")
            sys.exit(1)
