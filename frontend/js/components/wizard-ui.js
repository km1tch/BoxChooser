...
class WizardUI {
  constructor(containerId, options = {}) {
    ...
    this.rulesManager = options.rulesManager || null;  // Accept passed rules manager
  ...
  attachEventListeners() {
    ...
    if (findBtn) {
      let findBoxesInProgress = false;
      findBtn.addEventListener("click", async () => {
        if (findBoxesInProgress) return;
        findBoxesInProgress = true;
        try {
          await this.findBestBoxes();
        } finally {
          findBoxesInProgress = false;
        }
      });
    }
  }
  ...
  async showPackingLevels() {
    // Use the passed rules manager or create one if not provided
    let rulesManager = this.rulesManager;
    if (!rulesManager && typeof PackingRulesManager !== "undefined") {
