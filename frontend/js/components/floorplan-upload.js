...
export class FloorplanUpload {
    ...
    this.container.innerHTML = `
            <div class="floorplan-upload">
                ...
                <div class="upload-preview" id="upload-preview" style="display: none;">
                    <img id="preview-image" src="" alt="Floorplan preview" style="max-height: 400px;">
  ...
  handleFile(file) {
    ...
    if (!this.options.allowedTypes.includes(file.type)) {
      this.showError(
        "Invalid file type. Please upload PNG, JPEG, GIF, BMP, TIFF, or WebP files."
      );
      return;
    }
  ...
  async uploadFile() {
    ...
    try {
      ...
      if (result.original_type !== result.saved_type) {
        successMessage += ` (converted to ${
          result.saved_type === "image/png" ? "PNG" : result.saved_type
        })`;
      }
  ...
  async checkExistingLocations() {
    try {
      const response = await this.authenticatedFetch(
        `/api/store/${this.storeId}/box-locations`
      );
      ...
      // Silently fail - assume no existing locations
      return false;
    }
  }
