...
echo "Renaming JS files (excluding components)..."
find "$FRONTEND_DIR/js" -name "*.js" -type f | while read js_file; do
    # Skip component files that are dynamically imported
    if echo "$js_file" | grep -q "/js/components/"; then
        echo "  Skipping component: $(basename $js_file)"
        continue
    fi
